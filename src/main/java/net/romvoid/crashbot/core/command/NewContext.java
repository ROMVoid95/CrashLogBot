...
package net.romvoid.crashbot.core.command;
...
import net.romvoid.crashbot.core.command.argument.Arguments;
import net.romvoid.crashbot.core.command.argument.MarkedBlock;
import net.romvoid.crashbot.core.command.argument.Parser;
import net.romvoid.crashbot.core.command.argument.split.StringSplitter;
...
public class NewContext {
    ...
    /**
     * Attempts to parse an argument with the provided {@link net.romvoid.crashbot.core.command.argument.Parser parser}.
     ...
     *
     * @param parser Parser to use.
     * @param failureMessage Message to provide to the {@link net.romvoid.crashbot.core.command.argument.ArgumentParseError error}
     ...
     * <pre><code>
     * List&lt;Integer&gt; ints = context.many({@link net.romvoid.crashbot.core.command.argument.Parsers#strictInt()} Parsers.strictInt()});
     * assertEquals(ints.size(), 2);
     * assertEquals(ints.get(0), 1);
     * assertEquals(ints.get(1), 2);
     * String string = context.argument({@link net.romvoid.crashbot.core.command.argument.Parsers#string() Parsers.string()});
     ...
     * <pre><code>
     * List&lt;Integer&gt; ints = context.takeUntil({@link net.romvoid.crashbot.core.command.argument.Parsers#strictInt() Parsers.strictInt()}, {@link net.romvoid.crashbot.core.command.argument.Parsers#strictInt() Parsers.strictInt()}.{@link Parser#filter(java.util.function.Predicate) filter(x-&gt;x &lt; 0)});
     * assertEquals(ints.size(), 2);
     * assertEquals(ints.get(0), 1);
     * assertEquals(ints.get(1), 2);
     * Integer last = context.argument({@link net.romvoid.crashbot.core.command.argument.Parsers#strictInt() Parsers.strictInt()});
     * assertEquals(last, -1);
     * </code></pre>
     *
